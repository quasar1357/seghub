...
def get_pca_features(features, num_components=2):
    '''
    Takes linear features or a feature space,
    returns the principal components of the features (normalized to 0-1).
    INPUT:
    ...
    OUTPUT:
        pc_normalized (np.ndarray): normalized principal components.
                                    shape (H, W), or (H*W) accoring to input features
    '''
    ...
    princ_comp = pca.fit_transform(features_linear)
    pc_normalized = (princ_comp - np.min(princ_comp)) / (np.max(princ_comp) - np.min(princ_comp))
    if len(features.shape) > 2:
...
def get_kmeans_clusters(features, num_clusters=3):
    ...
    OUTPUT:
        cluster_labels (np.ndarray): cluster labels.
                                     shape (H*W), or (H, W) according to input features
    '''
    if len(features.shape) > 2:
        # features = np.moveaxis(features, -1, 0)
        features_linear = features.reshape(-1, features.shape[-1])
...
def features_combinator(image, features_func_list, features_cfg_list,
                        num_pcs_list=None, feature_smoothness_list=None, img_as_feature=False):
    '''
    Takes an image and a list of feature functions and their configurations,
    returns a combined feature space.
    INPUT:
    ...
    for i, features_func in enumerate(features_func_list):
        ...
        if feature_smoothness_list is not None and feature_smoothness_list[i] > 0:
            ...
            filter_footprint = morphology.disk(feature_smoothness_list[i])

...
def get_vgg16_feature_space(image, layer_list=[0], scalings=[1,2], model_name="vgg16", rgb=False):
    '''
    ...
    features = filter_image_multichannels_rgb(image, model, scalings=scalings,
                                          order=1, image_downsample=1, rgb=rgb)
...
def get_vgg16_features_targets(image, labels, layer_list=[0], scalings=[1,2], model_name="vgg16", rgb=False):
    '''
    ...
    '''
    feature_space = get_vgg16_feature_space(image, layer_list=layer_list, scalings=scalings, model_name=model_name, rgb=rgb)
    features_annot, targets = get_features_targets(feature_space, labels)
    return features_annot, targets

def get_vgg16_pixel_features(image, layer_list=[0], scalings=[1,2], model_name="vgg16", rgb=False):
    '''
    Takes an image and extracts features using a vgg16 model;
...
def filter_image_multichannels_rgb(image, hookmodel, scalings=[1], order=0,
    ...
    if image.ndim == 2:
        ...
        if not image.ndim == 3 and image.shape[0] == input_channels:
            raise ValueError("If RGB mode is chosen, the number of channels (first dimension) of the image must be 3.")
        # print("Using RGB")
        image = image[:, ::image_downsample, ::image_downsample]
        image_series = [image]
    elif image.ndim == 3:

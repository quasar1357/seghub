...
from seghub.util_funcs import (test_img_labels_batch_shapes, reshape_patches_to_img,
                               calculate_padding, get_features_targets)
from seghub.classif_utils import get_pca_features


def train_segforest(image_batch, labels_batch, features_func, features_cfg=None,
                    print_steps=False, random_state=0,
                    pcs_as_features=False, feature_smoothness=False, img_as_feature=False):
    '''
    Takes an image batch and a label batch, extracts features using the given function,
    and trains a random forest classifier.
    INPUT:
        image_batch (list of np.ndarrays or np.ndarray): list/batch of images.
                                                         Each image has shape (H, W, C) or (H, W)
        labels_batch (list of np.ndarrays or np.ndarray): list/batch of labels.
                                                          Each label has shape (H, W)
        ...
        features_cfg (dict): configuration for the feature extraction function
                             (parameters of the function given as key-value pairs)
        print_steps (bool): whether to print progress
        random_state (int): random state for the random forest classifier
        pcs_as_features (bool or int): number of principal components to use as features
                                       (or False to turn off)
        feature_smoothness (bool or int): size of the median filter footprint for
                                          smoothening the features (or False to turn off)
        img_as_feature (bool): whether to add the original image as a feature
                               (each channel is added as a feature)
    OUTPUT:
    ...
    t_start = time()
    # Iterate over the images and their labels; extract features & targets for each annotated pixel
    for image, labels in zip(image_batch, labels_batch):
        ...
        if feature_smoothness:
            ...
            filter_footprint = morphology.disk(feature_smoothness)
...
def predict_segforest_single_image(image, random_forest, features_func, features_cfg=None,
                                   pred_per_patch=False, patch_size=(14,14),
                                   pcs_as_features=False, feature_smoothness=False,
                                   img_as_feature=False, pred_smoothness=False):
    '''
    Takes an image and a trained random forest classifier,
    extracts features using the given function, and predicts labels.
    INPUT:
        ...
        features_cfg (dict): configuration for the feature extraction function
                             (parametes of the function given as key-value pairs)
        ...
        img_as_feature (bool): whether to add the original image as a feature
                               (each channel is added as a feature)
        pred_smoothness (bool or int): size of the majority filter footprint
                                       for smoothening the prediction (or False to turn off)
    OUTPUT:
    ...
    if feature_smoothness:
        if len(features.shape) < 3:
            raise ValueError("Smoothening of features only possible if " +
                             "extracted as feature space.")
        features = np.moveaxis(features, 2, 0)
        filter_footprint = morphology.disk(feature_smoothness)
        features = np.array([filters.median(f, footprint=filter_footprint)
                             for f in features])
        features = np.moveaxis(features, 0, 2)
    # Add original image as feature if desired
    if img_as_feature:
        if len(features.shape) < 3:
            raise ValueError("Adding image as feature only possible if " + 
                             "features are extracted as feature space.")
        if len(image.shape) == 2:
            image = np.expand_dims(image, axis=2)
...
def predict_segforest(img_batch, random_forest, features_func, features_cfg=None,
                      pred_per_patch=False, patch_size=(14,14), print_steps=False,
                      pcs_as_features=False, feature_smoothness=False,
                      img_as_feature=False, pred_smoothness=False):
    '''
    Takes an image batch and a trained random forest classifier,
    extracts features using the given function, and predicts labels for all images.
    INPUT:
        img_batch (list of np.ndarrays or np.ndarray): list/batch of images to predict on.
                                                       Each image has shape (H, W, C) or (H, W)
        random_forest (RandomForestClassifier): trained random forest classifier
        features_func (function): function to extract features from an image
            must take an image of shape (H, W, C) or (H, W)
    ...
    for i, image in enumerate(img_batch):
        ...
        pred_batch[i] = predict_segforest_single_image(
            image, random_forest, features_func, features_cfg,
            pred_per_patch=pred_per_patch, patch_size=patch_size,
            pcs_as_features=pcs_as_features, feature_smoothness=feature_smoothness,
            img_as_feature=img_as_feature, pred_smoothness=pred_smoothness)

    return pred_batch

def selfpredict_segforest_single_image(image, labels, features_func, features_cfg=None,
                                       random_state=0,
                                       pcs_as_features=False, feature_smoothness=False,
                                       img_as_feature=False, pred_smoothness=False):
    '''
    Takes an image and labels, extracts features using the given function,
    trains a random forest classifier based on the labels,
    and predicts labels for the entire image.
    Extracts features only once.
    INPUT:
        image (np.ndarray): image to predict on. Shape (H, W, C) or (H, W)
...
def segment_segforest(train_image_batch, labels_batch, pred_image_batch,
                      features_func, features_cfg=None,
                      print_steps=False, random_state=0,
                      pcs_as_features=False, feature_smoothness=False,
                      img_as_feature=False, pred_smoothness=False):
    '''
    Takes an image batch and a label batch, extracts features using the given function,
    trains a random forest classifier based on the labels,
    and predicts labels for the entire image batch or on a separate image batch if given.
    INPUT:
        train_image_batch (list of np.ndarrays or np.ndarray): list/batch of images.
                                                               Each image has shape (H, W, C) or (H, W)
        labels_batch (list of np.ndarrays or np.ndarray): list/batch of labels.
                                                          Each label has shape (H, W)
        pred_image_batch (list of np.ndarrays or np.ndarray): list/batch of images to predict on.
                                                              Each image has shape (H, W, C) or (H, W)
        ...
        pred_smoothness (bool or int): size of the majority filter footprint
                                       for smoothening the prediction (or False to turn off)
    OUTPUT:
        pred_batch (np.ndarray): predicted labels. Shape (N, H, W) where N is the number
                                 of images in the batch to predict
    '''
    if features_cfg is None:
        features_cfg = {}
    if pred_image_batch is None:
        pred_image_batch = train_image_batch
    rand_for = train_segforest(train_image_batch, labels_batch,
                               features_func=features_func, features_cfg=features_cfg,
                               print_steps=print_steps,
                               random_state=random_state,
                               pcs_as_features=pcs_as_features,
                               feature_smoothness=feature_smoothness,
                               img_as_feature=img_as_feature)
    pred_batch = predict_segforest(pred_image_batch, rand_for,
                                   features_func=features_func, features_cfg=features_cfg,
                                   print_steps=print_steps,
                                   pcs_as_features=pcs_as_features,
                                   feature_smoothness=feature_smoothness,
